...
public class PictureInPicturePlugin extends Plugin
{
	...
	{
		if (RuneLiteProperties.DEBUG_MODE)log.debug("PIP started!");
	}

	@Override
	protected void shutDown() throws Exception
	{
		if (RuneLiteProperties.DEBUG_MODE)log.debug("PIP stopped!");
		destroyPip();
	}
	...
	public void onClientTick(ClientTick event)
	{
		...
		if (clientTick % config.redrawRate().getId() == 0)
		{
			...
			{
				updatePip();
				//if (RuneLiteProperties.DEBUG_MODE)log.debug("PIP updated");
			}
		}
		clientTick++;
		...
		SwingUtilities.invokeLater(new Runnable()
		{
			...
			public void run()
			{
				if (!pipUp)
				{
					...
					if (RuneLiteProperties.DEBUG_MODE)log.debug("Initializing PIP");
					...
					if (RuneLiteProperties.DEBUG_MODE)log.debug("PIP initialized");
				}
			}
		});
	}
	...
	private void destroyPip()
	{
		...
		if (pipFrame != null)
		{
			...
			pipFrame = null;
			pipUp = false;
			if (RuneLiteProperties.DEBUG_MODE)log.debug("PIP destroyed");
		}
		...
		for (Frame frame : Frame.getFrames())
		{
			if (frame.getTitle() != null && frame.getTitle().equals("Picture in Picture"))
			{
				...
				frame.dispose();
				frame = null;
				if (RuneLiteProperties.DEBUG_MODE)log.debug("PIP cleanup");
			}
		}
	}
